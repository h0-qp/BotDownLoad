            ...
            if result.get("code") == 0:
                ...
                if "images" in body:
                    return {
                        "type": "images",
                        "images": body["images"],
                        "audio": body.get("music")
                    }
                else:
                    return {
                        "type": "video",
                        "video_url": body.get("play"),
                        "title": body.get("title")
                    }
            return None

async def fetch_instagram_data(url: str) -> list:
    """استخراج بيانات إنستجرام عبر الهندسة العكسية"""
...
async def start_command(client, message):
    ...
    # 1. تسجيل العضو وإرسال إشعار للمالك
...
async def media_downloader_router(client, message):
    ...
    # التأكد من تسجيل المستخدم في قاعدة البيانات في حال لم يرسل /start مسبقاً
    users = db.get("users")
    if user_id not in users:
        users.append(user_id)
        db.set("users", users)
    
    # التحقق من الاشتراك الإجباري قبل توفير الخدمة
    channel = db.get("force_channel")
    if not await is_subscribed(client, user_id):
        btn = InlineKeyboardMarkup([
            [InlineKeyboardButton("📢 اضغط هنا للاشتراك", url=f"https://t.me/{channel.replace('@', '')}")]
